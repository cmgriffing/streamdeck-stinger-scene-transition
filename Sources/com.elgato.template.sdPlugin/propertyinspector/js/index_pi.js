...
obs.on("ConnectionClosed", reconnectToObs);

obs.on("ConnectionError", reconnectToObs);
...
$SD.on("connected", async (jsn) => {
  try {
    ...
    const sceneOptions = scenes.map(function (scene) {
      let selected = "";
      if (settings.scene === scene.sceneName) {
        selected = "selected";
      }
      return `<option value="${scene.sceneName}" ${selected}>${scene.sceneName}</option>`;
    });
    sceneElement.innerHTML = sceneOptions.join("");

    const sourceElement = document.querySelector("#source");

    async function updateSourceOptions(sceneName) {
      const sourceOptions = (
        await obs.call("GetSceneItemList", { sceneName })
      ).sceneItems.map(function (source) {
        let selected = "";
        if (settings.source === source.sceneItemId) {
          selected = "selected";
        }
        return `<option value="${source.sceneItemId}" ${selected}>${source.sourceName}</option>`;
      });
      sourceElement.innerHTML = sourceOptions.join("");
    }

    if (settings.scene) {
      updateSourceOptions(settings.scene);
    }
  } catch (e) {
...
$SD.on("piDataChanged", (returnValue) => {
  ...
  if (returnValue.key === "scene" && settings.scene !== returnValue.value) {
    updateSourceOptions(returnValue.key);
  }

  if (returnValue.key === "source") {
    returnValue.value = parseInt(returnValue.value);
  }
